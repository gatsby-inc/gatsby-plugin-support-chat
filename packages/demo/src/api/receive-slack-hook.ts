...
async function setKey(key:string, value, cacheTime){
  let now = new Date().getTime()
  let expire = cacheTime === 0 ? false : cacheTime * 1000 * 60
  cache.setKey(key, {
    expire: expire === false ? false : now + this.expire,
    data: value
  })
}
...
export default async function handler(
  ...
  res: GatsbyFunctionResponse
    ...
    if(req.body.message.sub_type ===  "message_replied") {
      await setKey(threadTs, message, 10)
    }
